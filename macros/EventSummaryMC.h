...
#ifndef EventSummaryMC_h
#define EventSummaryMC_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   EventSummaryMC(TTree *tree=0);
   virtual ~EventSummaryMC();
...
#ifdef EventSummaryMC_cxx
EventSummaryMC::EventSummaryMC(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../files/anaTree.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../files/anaTree.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("../files/anaTree.root:/anatree");
...
EventSummaryMC::~EventSummaryMC()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t EventSummaryMC::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t EventSummaryMC::LoadTree(Long64_t entry)
{
...
void EventSummaryMC::Init(TTree *tree)
{
...
void EventSummaryMC::Show(Long64_t entry)
{
...
Int_t EventSummaryMC::Cut(Long64_t entry)
{
...
double EventSummaryMC::pointDistance(double x1, double y1, double z1, double x2, double y2, double z2){

    double dist = sqrt( pow(x2-x1,2) + pow(y2-y1,2) + pow(z2-z1,2));
    return dist;}


#endif // #ifdef EventSummaryMC_cxx
